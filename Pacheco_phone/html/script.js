$(document).ready(function() {
    ...
    window.addEventListener('message', function(e) {
        ...
        // ==========================================
            ...
            // Guardar e limpar backgrounds para o jogo ficar visível
            let savedBg = $(".ecra").css("background-image");
            $(".ecra").data("saved-bg", savedBg).css("background-image", "none");
            $("#lockscreen").css("background-image", "none");

            $("#telemovel").addClass("camara-ativa");

            // Mostrar overlay como flex (não block)
            $("#overlay-camara").css({ "display": "flex", "opacity": 0 }).animate({ opacity: 1 }, 200);

            // Esconder todos os elementos irmãos visíveis
            elementosCamaraEscondidos = [];
            $("#overlay-camara").siblings().filter(":visible").each(function() {
                elementosCamaraEscondidos.push($(this));
                $(this).hide();
            });
        }

        // ==========================================
        // MODO CÂMARA DESATIVADO
        // ==========================================
        else if (d.action === "esconderCamara") {
            // Restaurar backgrounds
            let savedBg = $(".ecra").data("saved-bg");
            if (savedBg && savedBg !== "none") {
                $(".ecra").css("background-image", savedBg);
            } else {
                let wall = localStorage.getItem('pacheco_wallpaper') || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000';
                $(".ecra").css("background-image", "url('" + wall + "')");
            }
            let wall = localStorage.getItem('pacheco_wallpaper') || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000';
            $("#lockscreen").css("background-image", "url('" + wall + "')");

            $("#overlay-camara").fadeOut(200);
            $("#telemovel").removeClass("camara-ativa");

            // Restaurar elementos escondidos
            if (elementosCamaraEscondidos && elementosCamaraEscondidos.length > 0) {
                elementosCamaraEscondidos.forEach(function($el) { $el.show(); });
            }
            elementosCamaraEscondidos = [];
        }
    ...
    // Botão de rodar câmara (frente/trás)
