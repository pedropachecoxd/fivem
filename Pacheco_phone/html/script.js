$(document).ready(function() {
    ...
    window.addEventListener('message', function(e) {
        ...
        // ==========================================
            ...
            // Guardar wallpaper e limpar fundo
            let savedBg = $(".ecra").css("background-image");
            $(".ecra").data("saved-bg", savedBg)
                    .css({ "background-color": "transparent", "background-image": "none" });
            $("#lockscreen").css({ "background-color": "transparent", "background-image": "none" });

            // Adicionar classe para CSS transparente
            $("#telemovel").addClass("camara-ativa");

            // Mostrar overlay com flex
            $("#overlay-camara")
                .addClass("ativo")
                .css("display", "flex")
                .hide()
            ...
            let wall = localStorage.getItem('pacheco_wallpaper')
                    || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000';

            $(".ecra").css({
                "background-color": "var(--bg-base)",
                "background-image": (savedBg && savedBg !== "none") ? savedBg : "url('" + wall + "')"
            });
            $("#lockscreen").css("background-image", "url('" + wall + "')");

            // Se veio URL de foto, meter no campo de compose
            if (d.photoUrl && d.photoUrl !== "" && d.photoUrl !== "erro") {
                $("#fg-compose-url").val(d.photoUrl).trigger('input');
            }
