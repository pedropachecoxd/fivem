$(document).ready(function() {
    ...
    let modoCamara = "tras"; // "tras" ou "selfie"
    let camaraAtiva = false;
    ...
    // ==========================================
    // CÂMARA — SISTEMA REFEITO (IMERSÃO MÁXIMA)
    // ==========================================

    function abrirOverlayCamara() {
        camaraAtiva = true;
        modoCamara = "tras";
        atualizarBadgeCamara();

        // Telemóvel fica invisível (mas mantém pointer-events:none via CSS)
        $("#telemovel").addClass("camara-ativa");

        // Overlay em fixed cobre o ecrã INTEIRO do jogo
        $("#overlay-camara").addClass("ativo").hide().fadeIn(200);
        $("#cam-mode-badge").addClass("visivel");
    }

    function fecharOverlayCamara(photoUrl) {
        camaraAtiva = false;

        // Esconder overlay
        $("#overlay-camara").removeClass("ativo").fadeOut(150);
        $("#cam-mode-badge").removeClass("visivel");

        // Restaurar telemóvel
        $("#telemovel").removeClass("camara-ativa");

        // Se veio URL de foto, guardar na galeria E meter no compose
        if (photoUrl && photoUrl !== "" && photoUrl !== "erro") {
            adicionarFotoGaleria(photoUrl);
            $("#fg-compose-url").val(photoUrl).trigger('input');
            mostrarNotificacao('success', '📸 Foto guardada!');
        }
    }

    function atualizarBadgeCamara() {
        let texto = modoCamara === "selfie" ? "🤳 SELFIE" : "📷 TRASEIRA";
        $("#cam-mode-badge").text(texto);
    }

    // Botão abrir câmara (no Fotogram compose)
    $(document).on('click', '#btn-fg-abrir-camara', function(e) {
        e.preventDefault();
        $.post('https://pacheco_phone/tirarFoto', JSON.stringify({}), function() {});
        // O Lua vai responder com "mostrarCamara"
    });

    // Botão capturar foto (obturador grande)
    $(document).on('click', '#btn-fg-capturar', function(e) {
        ...
        $(this).addClass('flash');
        setTimeout(() => $(this).removeClass('flash'), 400);

        $.post('https://pacheco_phone/capturarFoto', JSON.stringify({}), function(data) {
            if (data && data.url && data.url !== "") {
                fecharOverlayCamara(data.url);
            } else {
                mostrarNotificacao('error', 'Erro ao capturar foto.');
                fecharOverlayCamara(null);
            }
        });
    });

    // Botão cancelar câmara (X)
    ...
    // Botão rodar câmara (frente/trás)
    ...
    // Preview ao colar URL manualmente
    $("#fg-compose-url").on('input', function() {
        let url = $(this).val().trim();
        if (url) {
            $("#fg-preview-img").attr("src", url).show();
            $("#fg-preview-placeholder").hide();
            $("#fg-preview-label").text("Pré-visualização");
        } else {
            $("#fg-preview-img").hide();
            $("#fg-preview-placeholder").show();
            $("#fg-preview-label").text("Sem imagem selecionada");
        }
    });

    // ==========================================
    // MENSAGENS DO FIVEM (postMessage)
    // ==========================================
    window.addEventListener('message', function(e) {
        ...
        // CÂMARA — ativar overlay imersivo
        else if (d.action === "mostrarCamara") {
            abrirOverlayCamara();
        }

        // CÂMARA — fechar overlay
        else if (d.action === "esconderCamara") {
            fecharOverlayCamara(d.photoUrl || null);
        }
    });
    ...
    $(document).on('click', '.app', function() {
        ...
        else if (app === "fotografias") { carregarGaleriaFotos(); }
        else if (app === "paginasamarelas") {
            $(".pa-tab-btn").removeClass("active"); $(".pa-tab-btn[data-tab='lista']").addClass("active");
            $(".pa-tab-content").hide(); $("#pa-tab-lista").show();
            carregarPaginasAmarelas();
        }
    });
    ...
    $("#btn-fg-novo-post").click(function() {
        $("#fotogram-feed-view").hide();
        $("#fotogram-compose-view").show();
        $("#fg-compose-url, #fg-compose-caption").val("");
        $("#fg-preview-img").attr("src","").hide();
        $("#fg-preview-placeholder").show();
        $("#fg-preview-label").text("Sem imagem selecionada");
    });
    ...
    // ==========================================
    // FOTOGRAFIAS — GALERIA LOCAL
    // ==========================================
    let galeriaFotos = JSON.parse(localStorage.getItem('pacheco_fotos') || '[]');
    let fotoViewerIndex = -1;

    function salvarGaleriaFotos() {
        localStorage.setItem('pacheco_fotos', JSON.stringify(galeriaFotos));
    }

    function adicionarFotoGaleria(url) {
        if (!url || url === '' || url === 'erro') return;
        let foto = { url: url, data: new Date().toISOString() };
        galeriaFotos.unshift(foto);
        salvarGaleriaFotos();
        mostrarNotificacao('success', '📷 Foto guardada na galeria!');
    }

    function carregarGaleriaFotos() {
        let grid = $("#fotos-grid");
        let empty = $("#fotos-empty");
        grid.empty();

        if (galeriaFotos.length === 0) {
            grid.hide(); empty.show();
            $("#fotos-count").text("0 fotos");
            return;
        }
        empty.hide(); grid.show();
        $("#fotos-count").text(galeriaFotos.length + " foto" + (galeriaFotos.length !== 1 ? "s" : ""));

        galeriaFotos.forEach(function(foto, idx) {
            let thumb = $(`
                <div class="fotos-thumb" data-idx="${idx}">
                    <img src="${foto.url}" onerror="$(this).parent().hide()">
                    <div class="fotos-thumb-overlay"></div>
                </div>
            `);
            thumb.on('click', function() { abrirFotoViewer(idx); });
            grid.append(thumb);
        });
    }

    function abrirFotoViewer(idx) {
        if (idx < 0 || idx >= galeriaFotos.length) return;
        fotoViewerIndex = idx;
        let foto = galeriaFotos[idx];
        $("#fotos-viewer-img").attr("src", foto.url);
        let d = new Date(foto.data);
        let dateStr = d.toLocaleDateString('pt-PT', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
        $("#fotos-viewer-data").text(dateStr);
        $("#fotos-viewer").fadeIn(150);
    }

    $("#btn-fotos-fechar-viewer").click(function() {
        $("#fotos-viewer").fadeOut(150);
        fotoViewerIndex = -1;
    });

    $("#btn-fotos-apagar").click(function() {
        $("#modal-fotos-apagar").show();
    });

    $("#btn-fotos-cancelar-apagar").click(function() {
        $("#modal-fotos-apagar").hide();
    });

    $("#btn-fotos-confirmar-apagar").click(function() {
        if (fotoViewerIndex < 0) return;
        galeriaFotos.splice(fotoViewerIndex, 1);
        salvarGaleriaFotos();
        $("#modal-fotos-apagar").hide();
        $("#fotos-viewer").fadeOut(150);
        fotoViewerIndex = -1;
        carregarGaleriaFotos();
        mostrarNotificacao('success', 'Foto apagada.');
    });

    $("#btn-fotos-partilhar").click(function() {
        if (fotoViewerIndex < 0) return;
        let foto = galeriaFotos[fotoViewerIndex];
        // Copiar URL para clipboard (best effort no NUI)
        try { navigator.clipboard.writeText(foto.url); mostrarNotificacao('success', '🔗 URL copiada!'); }
        catch(e) { mostrarNotificacao('success', '🔗 URL: ' + foto.url.substring(0, 40) + '...'); }
    });

    // Publicar no Fotogram diretamente da galeria
    $("#btn-fotos-postar-fotogram").click(function() {
        if (fotoViewerIndex < 0) return;
        let foto = galeriaFotos[fotoViewerIndex];
        // Ir para Fotogram compose com URL preenchida
        $(".system-app, .app-screen").hide(); appAberta = "fotogram";
        if (!fotogramAccount) {
            $("#app-fotogram").show(); $("#fotogram-auth-view").show(); $("#fotogram-feed-view").hide();
            mostrarNotificacao('warning', 'Faz login no Fotogram primeiro.');
        } else {
            $("#app-fotogram").show(); $("#fotogram-auth-view").hide();
            $("#fotogram-feed-view").hide(); $("#fotogram-compose-view").show();
            $("#fg-compose-url").val(foto.url).trigger('input');
            $("#fg-compose-caption").val("");
        }
        $("#fotos-viewer").hide(); fotoViewerIndex = -1;
    });

    // Botão câmara no estado vazio + header
    $("#btn-fotos-nova, #btn-fotos-abrir-camara").click(function() {
        $.post('https://pacheco_phone/tirarFoto', JSON.stringify({}), function() {});
    });

    // Ao fechar a câmara com foto, adicionar à galeria
    // (integração com fecharOverlayCamara — intercetada abaixo)

    // ==========================================
    // PÁGINAS AMARELAS — REDESIGN PREMIUM
    // ==========================================
    let paCatAtiva = "all";
    let paSearchQuery = "";
    let paEmpresaModalData = null;

    // Funções de badge por categoria
    function paCatBadgeClass(cat) {
        let map = { mecanico: 'pa-badge-mecanico', medico: 'pa-badge-medico', advogado: 'pa-badge-advogado', outro: 'pa-badge-outro' };
        return map[cat] || 'pa-badge-outro';
    }
    function paCatLabel(cat) {
        let map = { mecanico: 'Mecânico', medico: 'Médico', advogado: 'Advogado', outro: 'Outro' };
        return map[cat] || (cat || 'Outro');
    }
    function paCatIcon(cat) {
        let map = { mecanico: 'fa-wrench', medico: 'fa-stethoscope', advogado: 'fa-gavel', outro: 'fa-ellipsis-h' };
        return map[cat] || 'fa-building';
    }
    function paCatAccent(cat) {
        let map = { mecanico: '#f59e0b', medico: '#10b981', advogado: '#818cf8', outro: '#94a3b8' };
        return map[cat] || '#eab308';
    }

    function carregarPaginasAmarelas(category) {
        let cat = category !== undefined ? category : paCatAtiva;
        paCatAtiva = cat;

        $.post('https://pacheco_phone/getPaginasAmarelas', JSON.stringify({ category: cat }), function(empresas) {
            let container = $("#pa-lista-container");
            container.empty();

            // Filtrar por pesquisa
            if (paSearchQuery && empresas) {
                let q = paSearchQuery.toLowerCase();
                empresas = empresas.filter(function(e) {
                    return (e.business_name || '').toLowerCase().includes(q) ||
            ...
            if (!empresas || empresas.length === 0) {
                container.append(`
                    <div class="pa-empty-state">
                        ...
                        <p>Sem resultados</p>
                        <span>${paSearchQuery ? 'Tenta outra pesquisa' : 'Nenhuma empresa nesta categoria'}</span>
                    </div>
                `);
                return;
            }
            ...
            empresas.forEach(function(e) {
                let cat = e.category || e.job || 'outro';
                let badgeClass = paCatBadgeClass(cat);
                let catLabel = paCatLabel(cat);
                let accent = paCatAccent(cat);
                let descHtml = e.description ? `<p class="pa-card-desc">${e.description}</p>` : '';
                let telHtml = e.phone ? `<div class="pa-card-info"><i class="fas fa-phone"></i><span>${e.phone}</span></div>` : '';
                let locHtml = e.location ? `<div class="pa-card-info"><i class="fas fa-map-marker-alt"></i><span>${e.location}</span></div>` : '';

                let card = $(`
                    <div class="pa-empresa-card" style="--pa-accent:${accent}">
                        <div class="pa-card-top">
                            <span class="pa-card-nome">${e.business_name}</span>
                            <span class="pa-card-badge ${badgeClass}">${catLabel}</span>
                        </div>
                        ${descHtml}
                        <div class="pa-card-footer">
                            ${telHtml}
                            ${locHtml}
                            <i class="fas fa-chevron-right pa-card-arrow"></i>
                        </div>
                    </div>
                `);
                card.on('click', function() { abrirModalEmpresa(e); });
    ...
    function abrirModalEmpresa(e) {
        paEmpresaModalData = e;
        let cat = e.category || e.job || 'outro';
        let badgeClass = paCatBadgeClass(cat);
        let catLabel = paCatLabel(cat);

        $("#pa-modal-cat").attr("class", "pa-modal-cat-badge " + badgeClass).text(catLabel);
        $("#pa-modal-nome").text(e.business_name);
        $("#pa-modal-desc").text(e.description || '');

        if (e.phone) { $("#pa-modal-tel").text(e.phone); $("#pa-modal-tel-row").show(); }
        else { $("#pa-modal-tel-row").hide(); }
        if (e.location) { $("#pa-modal-loc").text(e.location); $("#pa-modal-loc-row").show(); }
        else { $("#pa-modal-loc-row").hide(); }

        if (!e.phone) { $("#pa-modal-btn-ligar").prop('disabled', true).css('opacity', 0.4); }
        else { $("#pa-modal-btn-ligar").prop('disabled', false).css('opacity', 1); }

        $("#pa-modal-empresa").fadeIn(150);
    }

    $("#pa-modal-btn-fechar").click(function() { $("#pa-modal-empresa").fadeOut(150); });
    $("#pa-modal-btn-ligar").click(function() {
        if (!paEmpresaModalData || !paEmpresaModalData.phone) return;
        $("#pa-modal-empresa").hide();
        window.tentarLigar(paEmpresaModalData.phone, paEmpresaModalData.business_name);
    });

    // Categorias filtragem
    $(document).on('click', '.pa-cat', function() {
        $(".pa-cat").removeClass("active"); $(this).addClass("active");
        paCatAtiva = $(this).data("cat");
        carregarPaginasAmarelas(paCatAtiva);
    });

    // Pesquisa em tempo real
    $(document).on('input', '#pa-search-input', function() {
        paSearchQuery = $(this).val().trim();
        carregarPaginasAmarelas(paCatAtiva);
    });

    // Tabs
    $(document).on('click', '.pa-tab-btn', function() {
        $(".pa-tab-btn").removeClass("active"); $(this).addClass("active");
        $(".pa-tab-content").hide();
        let tab = $(this).data("tab");
        $("#pa-tab-" + tab).fadeIn(150);
        if (tab === "lista") carregarPaginasAmarelas(paCatAtiva);
        else if (tab === "minha") carregarMinhaEmpresa();
    });

    // Seletor visual de categoria nos formulários
    $(document).on('click', '#pa-cat-select-reg .pa-cat-select-item', function() {
        $("#pa-cat-select-reg .pa-cat-select-item").removeClass("selected");
        $(this).addClass("selected");
        $("#pa-empresa-cat-reg").val($(this).data("val"));
    });
    $(document).on('click', '#pa-cat-select-edit .pa-cat-select-item', function() {
        $("#pa-cat-select-edit .pa-cat-select-item").removeClass("selected");
        $(this).addClass("selected");
        $("#pa-empresa-cat-edit").val($(this).data("val"));
    });

    // Atalho para ir para Registar
    $(document).on('pa-goto-reg', function() {
        $(".pa-tab-btn").removeClass("active"); $(".pa-tab-btn[data-tab='registar']").addClass("active");
        $(".pa-tab-content").hide(); $("#pa-tab-registar").fadeIn(150);
    });

    function carregarMinhaEmpresa() {
        $.post('https://pacheco_phone/getMinhaEmpresa', JSON.stringify({}), function(empresa) {
            if (empresa && empresa.business_name) {
                $("#pa-minha-vazia").hide();
                $("#pa-minha-form").show();
                $("#pa-empresa-nome-edit").val(empresa.business_name || "");
                $("#pa-empresa-desc-edit").val(empresa.description || "");
                $("#pa-empresa-tel-edit").val(empresa.phone || "");
                $("#pa-empresa-loc-edit").val(empresa.location || "");
                // Selecionar categoria visualmente
                let cat = empresa.category || '';
                $("#pa-cat-select-edit .pa-cat-select-item").removeClass("selected");
                $("#pa-cat-select-edit .pa-cat-select-item[data-val='" + cat + "']").addClass("selected");
                $("#pa-empresa-cat-edit").val(cat);
            } else {
                $("#pa-minha-form").hide();
                $("#pa-minha-vazia").show();
            }
    ...
    $("#btn-registar-empresa").click(function() {
        let nome = $("#pa-empresa-nome-reg").val().trim();
        let desc = $("#pa-empresa-desc-reg").val().trim();
        let tel = $("#pa-empresa-tel-reg").val().trim();
        let cat = $("#pa-empresa-cat-reg").val().trim();
        let loc = $("#pa-empresa-loc-reg").val().trim();
        if (!nome) { mostrarNotificacao('error', 'Nome da empresa obrigatório!'); return; }
        if (!cat) { mostrarNotificacao('error', 'Seleciona uma categoria!'); return; }
        ...
        $.post('https://pacheco_phone/registarEmpresa', JSON.stringify({ businessName: nome, description: desc, phone: tel, category: cat, location: loc }), function(res) {
            ...
            if (res.success) {
                mostrarNotificacao('success', '✅ ' + res.message);
                // Limpar form
                $("#pa-empresa-nome-reg, #pa-empresa-desc-reg, #pa-empresa-tel-reg, #pa-empresa-loc-reg").val("");
                $("#pa-cat-select-reg .pa-cat-select-item").removeClass("selected");
                $("#pa-empresa-cat-reg").val("");
                // Ir para lista
                $(".pa-tab-btn").removeClass("active"); $(".pa-tab-btn[data-tab='lista']").addClass("active");
                $(".pa-tab-content").hide(); $("#pa-tab-lista").fadeIn(150);
                carregarPaginasAmarelas("all");
            } else {
                mostrarNotificacao('error', res.message);
            }
        });
    });

    $("#btn-atualizar-empresa").click(function() {
        let nome = $("#pa-empresa-nome-edit").val().trim();
        let desc = $("#pa-empresa-desc-edit").val().trim();
        let tel = $("#pa-empresa-tel-edit").val().trim();
        let cat = $("#pa-empresa-cat-edit").val().trim();
        let loc = $("#pa-empresa-loc-edit").val().trim();
        if (!nome) { mostrarNotificacao('error', 'Nome obrigatório!'); return; }
        let btn = $(this); btn.prop('disabled', true);
        $.post('https://pacheco_phone/atualizarEmpresa', JSON.stringify({ businessName: nome, description: desc, phone: tel, category: cat, location: loc }), function(res) {
            btn.prop('disabled', false);
